...
if pipeline_records:
    ...
    WHEN MATCHED AND (
        ...
        OR target.run_end   IS DISTINCT FROM source.run_end
        OR target.duration_ms IS DISTINCT FROM source.duration_ms
        OR target.message   IS DISTINCT FROM source.message
...
if activity_records:
    ...
    WHEN MATCHED AND (
        target.status             IS DISTINCT FROM source.status
        OR target.activity_run_end IS DISTINCT FROM source.activity_run_end
        OR target.duration_ms     IS DISTINCT FROM source.duration_ms
        OR target.error_message   IS DISTINCT FROM source.error_message
    ) THEN UPDATE SET
        target.status             = source.status,
